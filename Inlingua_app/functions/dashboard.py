...
def dashboard(request):
    ...
    all_students = StudentTable.objects.filter(Q(user__is_active=False) | Q(payment_complited=False)).order_by('id')
    paginator = Paginator(all_students, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    # Trainer Head students list trainer_head_students_list_page_obj
    trainer_head_students_list = StudentTable.objects.filter(status='Verified').order_by('id')
